...
static size_t ucp_tag_pack_eager_first_dt(void *dest, void *arg)
{
    ...
    size_t result = sizeof(*hdr) + ucp_dt_pack(req->send.ep->worker, req->send.datatype,
                                               req->send.mem_type, hdr + 1, req->send.buffer,
...
static ucs_status_t ucp_tag_eager_zcopy_single(uct_pending_req_t *self)
{
    ...
    hdr.super.tag = req->send.tag.tag;
    ucs_status_t result = ucp_do_am_zcopy_single(self, UCP_AM_ID_EAGER_ONLY, &hdr, sizeof(hdr),
...
static ucs_status_t ucp_tag_eager_sync_zcopy_single(uct_pending_req_t *self)
{
    ...
    ucs_status_t result = ucp_do_am_zcopy_single(self, UCP_AM_ID_EAGER_SYNC_ONLY, &hdr, sizeof(hdr),
